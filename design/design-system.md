...
|-------|------|-----------|---------|
| **Roboto Condensed** (All Caps) | Заголовки всех уровней (H1–H5) | Bold / ExtraBold, uppercase | Сдержанный, технологичный, уверенный |
...
|-----|----|-----|---------------|------|
| `10` | `10` | `0.625rem` | `--text-10` | Sub-micro: PIK caption, мельчайшие подписи |
...
| `20` | `20` | `1.25rem` | `--text-20` | Stats hero, промежуточный heading-accent |
| `24` | `24` | `1.5rem` | `--text-24` | H4, H3 mobile |
| `25` | `25` | `1.5625rem` | `--text-25` | Marketing copy |
| `28` | `28` | `1.75rem` | `--text-28` | H3 mobile |
| `30` | `30` | `1.875rem` | `--text-30` | H2 mobile |
| `32` | `32` | `2rem` | `--text-32` | H3, H1 mobile |
| `40` | `40` | `2.5rem` | `--text-40` | heading-40, промежуточный |
...
| Класс | px desktop | px mobile | Вес | Line-height | Letter-spacing | Применение |
|-------|-----------|---------|----|------------|---------------|-----------|
| `.h1` | 80 | 32 | 800 ExtraBold | 1.0 | -0.02em | H1 Hero |
| `.h2` | 52 | 32 | 700 Bold | 1.08 | -0.02em | H2, заголовки страниц |
| `.h3` | 32 | 28 | 700 Bold | 1.16 | -0.015em | H3, подзаголовки секций |
| `.h4` | 24 | 24 | 700 Bold | 1.2 | -0.01em | H4, заголовки карточек |
| `.h5` | 18 | 16 | 700 Bold | 1.16 | -0.01em | H5, мелкие заголовки |

**CSS-классы:** `.h1`, `.h2`, `.h3`, `.h4`, `.h5` — определены в `packages/ui/src/styles/globals.css`. Автоматически применяют мобильный размер на `< 768px` и десктопный на `≥ 768px`. Включают font-family, font-weight, line-height, letter-spacing и text-transform.
...
- Для ИИ-агентов использовать маскот-изображения из `assets/ai-mascots/` (единый источник, симлинки в apps/*/public/).
...
| `--dot-gap` | `20px` | Расстояние между центрами точек (шаг сетки) |
...
### 8.14 New-notification state (карточка/строка «не просмотрено»)

Состояние для свежесозданного элемента (проект, артефакт, уведомление), которое пользователь ещё не открыл. Мягко привлекает внимание, снимается после первого клика.

#### Когда использовать

- Только что созданный проект в sidebar и в списке проектов, пока пользователь ни разу не открыл его.
- Артефакт, который эксперт сгенерировал, но пользователь не ознакомился.
- Системные уведомления / inbox-элементы.

**Не использовать** для статусов «в работе», «pending», «error». Это отдельные паттерны.

#### Состав паттерна

| Слой | Токен | Правило |
|------|-------|---------|
| Подложка (фон) | `var(--rm-yellow-10)` — `#FFFEF3` (light) / `#3D3300` (dark) | Самый бледный жёлтый из шкалы §1.1 — не конкурирует с акцентом |
| Пульсирующая обводка | `box-shadow: 0 0 0 {0→3}px rgba(255, 204, 0, {0→0.55})` | Анимируется через `@keyframes rm-new-pulse` |
| Длительность | `1.8s ease-in-out infinite` | Медленный ритм, не раздражает при фоновом взгляде |
| Reduced motion | `animation: none; box-shadow: 0 0 0 2px rgba(255, 204, 0, 0.4)` | Статичная обводка, без пульса |

#### CSS (в `globals.css` приложения)

```css
@keyframes rm-new-pulse {
  0%, 100% { box-shadow: 0 0 0 0 rgba(255, 204, 0, 0); }
  50%      { box-shadow: 0 0 0 3px rgba(255, 204, 0, 0.55); }
}
.rm-new-pulse {
  animation: rm-new-pulse 1.8s ease-in-out infinite;
}
@media (prefers-reduced-motion: reduce) {
  .rm-new-pulse {
    animation: none;
...
- Источник правды — клиентский флаг (для MVP — `localStorage`).
- Снимается **при первом клике** по элементу — переход на его страницу автоматически помечает его как «просмотрен».
- Повторно не навешивать, даже если пользователь не прочитал содержимое; визуальная пульсация — одноразовое приветствие «новизны».

#### Правила использования

- Только на **одном** иерархическом уровне одновременно: не помечайте одновременно родительский проект и его дочерний артефакт — это визуальный шум.
- Не использовать вместе с акцентной `hover`-обводкой. На hover пульс сохраняется, базовый hover-border не добавляется.
- Не более 3 непросмотренных элементов в одном скроллируемом списке — больше = шум, переходите к бэйджу-счётчику на родителе.

---

### 8.15 Message rise — real-time-only

Анимация появления новых сообщений в чате при отправке пользователем или прилёте ответа ИИ-эксперта/R-менеджера в реальном времени. **Не применяется** к исторически загруженным сообщениям (при открытии проекта или смене эксперта).

#### Когда использовать

- User-сообщение после отправки из инпута или клика на scenario/mode/HITL-picker.
- Assistant-сообщение, прилетевшее в ответ в real-time (включая thinking-сообщения глубокого цикла).
- Manager-сообщения R-менеджера во время онбординга-диалога.

**Не использовать** при первичном рендере истории — все bubble'ы появятся одновременно, это будет эффект «шума».

#### Параметры

| Параметр | Значение | Обоснование |
|----------|----------|-------------|
| Начало | `opacity: 0; transform: translateY(14px);` | Лёгкий старт снизу, как будто bubble «подлетает» из инпута |
| Конец | `opacity: 1; transform: translateY(0);` | Конечное положение |
| Длительность | `0.28s` | Достаточно, чтобы взгляд «поймал» анимацию, не тормозя флоу |
| Easing | `cubic-bezier(0.2, 0.8, 0.2, 1)` | Ease-out — мягкое приземление |
| Reduced motion | `animation: none` | Без анимации для пользователей с предпочтением |

#### CSS (в shared `@rocketmind/ui/styles`)

```css
@keyframes rm-message-rise {
  from { opacity: 0; transform: translateY(14px); }
  to   { opacity: 1; transform: translateY(0); }
}
.rm-message-rise {
  animation: rm-message-rise 0.28s cubic-bezier(0.2, 0.8, 0.2, 1);
}
@media (prefers-reduced-motion: reduce) {
  .rm-message-rise { animation: none; }
}
```
...
```tsx
// State: Set<string> id-шников сообщений, добавленных в real-time
const [freshIds, setFreshIds] = useState(new Set<string>());
...
// В bubble:
<div className={freshIds.has(msg.id) ? "rm-message-rise" : ""}>...</div>
```

#### Правила использования

- Только один раз при mount — сама CSS-анимация автоматически не повторяется.
- Не использовать вместе с `rm-new-pulse` на одном элементе — оба привлекают внимание, будет борьба.
- Применимо только к чат-бабблам (user, assistant, system, manager). Не применять к карточкам, кнопкам, другим UI-элементам.

---

### 8.16 Pipeline step dimmed state

Визуальное приглушение не-пройденных шагов последовательного процесса. Используется в sidebar-списке R-экспертов под проектом: пользователь сразу видит, какие этапы позади, текущий и ещё впереди.

#### Состояния

| Состояние | Visual | Opacity |
|-----------|--------|---------|
| `completed` | Нормальная яркость, `CheckCircle2` в жёлтом акценте | 100% |
| `in_progress` / `awaiting_validation` | Нормальная яркость, `CircleDashed` в foreground | 100% |
| **Current (активный у проекта)** | Нормальная яркость даже если `not_started` | 100% |
| `not_started` (не активный) | Приглушён | **50%** |
| `not_started` на hover | Восстанавливается | 100% |

#### Правила

- Текущий эксперт проекта (`project.current_expert_codename`) никогда не приглушается, даже если `session.status === "not_started"` — он следующий шаг, должен быть видимым.
- На hover — `opacity: 1` (чтобы пользователь мог прочитать непройденный шаг, если специально на него навёл).
- Не использовать приглушение в «dashboard»-сценариях: там не-пройденные этапы описываются прогресс-баром и бейджем `N/M`, а не приглушением.
...
### 9.2 Аккордеон — Product (минимальный)

Минимальный аккордеон для продуктовых страниц (секция «О продукте»). Одновременно открыт один элемент.

#### Анатомия

```
<div> (flex-col)
└── <button> (flex, items-start, gap-7, py-4, pr-4, border-[#404040])
    ├── <div> (flex-col, gap-4)
    │   ├── <span> заголовок (--font-mono-family, Label 16, uppercase)
    │   └── <p> описание (Copy 14, #939393) ← только если открыт
    └── <svg> +/− иконка (16×16, foreground)
```
...
| Свойство | Токен / значение | Описание |
|----------|-----------------|----------|
| Заголовок | `--font-mono-family`, Label 16, uppercase | tracking 0.02em, leading 1.12 |
| Описание (open) | Copy 14, `#939393` | tracking 0.01em, leading 1.32 |
| Иконка | `16×16`, `currentColor` | + (closed) / − (open), stroke 1.5 |
| Бордер | `#404040` | border-t border-b для первого, border-b для остальных |
| Padding | `py-4 pr-4` | Без pl — прижат к левому краю |
| Gap (title → description) | `16px` | Внутри открытого элемента |

#### Применение

| Экран | Секция |
|-------|--------|
| Страница продукта | «О продукте» |
...
#### Factoid-grid (section-level)

Сетка карточек-фактоидов **section-level**: живёт на уровне секции (`ArticleSection.factoids: FactoidCardData[]`), не как блок тела статьи. Рендерится строго в начале секции — между H2 и `blocks[]`. Размещение фиксировано, потому что 3-я карточка визуально вылезает в col-4 (правую aside-колонку) и сдвигает sticky-asides секции вниз — детерминированно посчитать смещение можно только когда фактоиды строго в начале.

`FactoidCardData = { id, number, text, accent }`. `accent: true` — жёлтая подложка `--rm-yellow-100` с тёмным текстом `#0A0A0A` (для number и text); по умолчанию — прозрачный фон, текст `--rm-gray-fg-main` / `--rm-gray-fg-sub`.

**Стиль карточки (по фигме `1067:7086`):** padding `28px`, gap `24px` между блоком цифры и текстом. Цифра — Roboto Condensed Bold 32px (mobile) / 52px (desktop), line-height 108%, tracking −2%, UPPER. Текст — Roboto 16px / 128%. Идентично фактоидам в hero продуктовых страниц (`ProductHero`), но без `label` — только `number` + `text`.

**Сетка и адаптив:**

| Брейкпоинт | Колонок | Поведение |
|------------|---------|-----------|
| <768 (mobile) | 1 | Карточки стеком вниз |
| 768–1279 (tablet) | 2 | 2 в ряд |
| ≥1280 (desktop unified-grid) | 3 | 3-я карточка визуально вылезает в col-4 (правая колонка), как wide-quote (negative `marginRight = asideW + 8`) |

**Линии решётки:** `gap-px` + `bg: var(--rm-gray-3)` на родителе + `outline: 1px solid var(--rm-gray-3)` — единая 1px-сетка вокруг и между карточек. Accent-карточка меняет свой bg на жёлтый, линии решётки рядом сохраняются.

**Влияние на правую колонку (sticky asides):** в 3-кол layout каждый ряд, где есть 3-я карточка, перекрывает aside-колонку. Sync в `article-page-client.tsx` суммирует высоты таких рядов и применяет это число как `padding-top` на `[data-section-aside-inner]` секции — sticky-карточки начинают свою зону ниже 3-х карточек фактоидов и не пересекаются с ними. Если, например, 5 карточек = 3+2: только row-1 имеет 3-ю → смещение = высота row-1; 6 карточек = 3+3 → смещение = высота row-1 + row-2.

**В админке:** кнопка «+ Добавить сетку фактоидов» появляется под инпутом H2 в редакторе секции. Когда сетка добавлена — per-card: стрелки ←→↑↓ для перестановки, sparkles для accent toggle, корзина для удаления карточки (удаление последней — убирает сетку целиком). «+ Карточка справа» (активна при <3 в ряду) и «+ Карточка ниже» (активна при ровно 3).

**Экспорт из `@rocketmind/ui`:** `FactoidGrid` (компонент) + `FactoidCardData` (тип).
...
- **Динамичность**: Логотипы загружаются серверным компонентом из папки `assets/clip-logos/` через `getPartnerLogos()` (Node.js `fs`). Добавление/удаление файла автоматически обновляет список.
...
| Действие | Инструкция |
|----------|-----------|
| **Добавить логотип** | Положить SVG-файл в `assets/clip-logos/`. Имя файла = slug в нижнем регистре, например `sberbank.svg`. Перезапустить dev-сервер. |
| **Удалить логотип** | Удалить файл из `assets/clip-logos/`. |
...
- **Директория логотипов:** `assets/clip-logos/`
...
#### Высота и внутренний скролл (`stickyTop` mode)

Когда `<GlossaryWidget stickyTop="0" />` встроен в grid рядом с колонкой статей:
...
- **Внутренний скролл** включается автоматически при переполнении. Шапка (название + ссылка на полный глоссарий + поле поиска) остаётся `sticky` к верху scroll-контейнера, чтобы поиск всегда был под рукой.
- **Скроллбар** — `.rm-scrollbar-white-2` (см. 6.3.5): 2px белая полоска `--rm-white` вплотную к правому краю плашки.
- Реализация: `<aside class="relative h-full">` → `<div class="absolute inset-0 overflow-y-auto rm-scrollbar-white-2">`. `absolute` убирает aside из обычного потока row-sizing, поэтому высоту строки диктует соседняя колонка со статьями.
