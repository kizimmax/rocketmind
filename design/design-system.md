...
1. Если элемент нажимается, вводит данные или является частью control-group, почти всегда это `4px`.
2. Если элемент является поверхностью и содержит контент внутри, почти всегда это `8px`.
3. `rounded-full` не используется для обычных кнопок, инпутов, карточек и таблиц.

#### Копирование в DS web

- Кнопка `Token` копирует CSS token: `--radius-sm`, `--radius-lg`, `--radius-full`
- Кнопка `Tailwind` копирует utility-класс: `rounded-sm`, `rounded-lg`, `rounded-full`
...
Компонент для немедленного бинарного on/off внутри настроек, service rows и compact-toolbar. Основа: control-токены `--border`, `--ring`, `--rm-gray-1`, `--rm-yellow-100` и внешний контекст строки.

**Ключевые сценарии:** `Default`, `Disabled`, `Sizes`, `Full width`, `Tooltip`, `Icon`.

#### Когда использовать

- Быстрые настройки, где изменение применяется сразу: уведомления, автозапуск, показывать / скрывать
- Full-width строки настроек внутри карточек, модальных окон и service panels
- Compact-toggle в toolbar, если у пользователя уже есть ясный контекст происходящего

> **Не использовать** для юридически значимых согласий и множественного выбора. Для этого есть `Checkbox`. Если нужно выбрать один вариант из группы, использовать `Radio`.

#### Размеры и форма

| Вариант | Rail | Thumb | Форма | Применение |
|---------|------|-------|-------|-----------|
| `default` | `36px × 20px` | `16px × 16px` | `rounded-full` / `--radius-full` | Основной размер для строк настроек и форм |
| `sm` | `28px × 16px` | `12px × 12px` | `rounded-full` / `--radius-full` | Compact UI: toolbar, плотные inline-строки |

#### Состояния

| Состояние | Токены | Описание |
|-----------|--------|----------|
| `unchecked / default` | `border: --border`, `bg: --rm-gray-1`, thumb `--foreground` | Нейтральный quiet-control без декоративного hover |
| `checked` | `border: --rm-yellow-100`, `bg: --rm-yellow-100`, thumb `--rm-yellow-fg` | Активное состояние и немедленно применённая настройка |
| `focus-visible` | `border: --ring` + outer ring 3px / 50% | Повторяет общую keyboard-focus логику Rocketmind |
| `disabled` | `opacity: 0.4`, `cursor: not-allowed` | Control остаётся видимым в структуре настроек, но недоступен |
| `full-width` | строка `border-border`, `bg-background` или `bg-rm-gray-1`, switch справа | Основная композиция для settings-list |
| `tooltip` | tooltip висит на label или helper-иконке | Сам switch не получает нового visual-стиля |
| `icon` | иконка в строке слева, цвет `currentColor` | Иконка только помогает быстрее считать смысл настройки |
...
className="
  ...
  rounded-full border border-border bg-rm-gray-1 p-[1px]
...
// Thumb
className="
  pointer-events-none block rounded-full bg-foreground
...
<label className="flex items-center justify-between gap-4">
  <span>Автоматически отправлять ссылку на оплату</span>
...
<label className="flex items-start justify-between gap-4 rounded-lg border border-border bg-background p-4">
  <span className="flex items-start gap-3">
    ...
    <span>Пуши о новых ответах агента</span>
  </span>
  <Switch />
</label>
```
...
- Switch используем только там, где изменение можно применить сразу и не нужно отдельное подтверждение через кнопку `Save`.
- Основная композиция Rocketmind — full-width row: слева label + optional description, справа switch.
- Tooltip и иконка остаются внешней композицией строки. Не вставлять текст, иконки или счетчики внутрь rail.
- Не добавлять отдельные hover-эффекты, glow, shadow или вторую акцентную палитру: для switch достаточно состояний `unchecked`, `checked`, `focus-visible`, `disabled`.
