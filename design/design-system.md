...
Единая числовая шкала `--rm-gray-{N}` — основа всех нейтральных решений. Каждый уровень несёт конкретную UI-роль. Используй `--rm-gray-{N}` напрямую — без обёрток `--muted`/`--accent`. shadcn-алиасы (`--muted`, `--accent`, `--secondary`) остаются в globals.css для совместимости с компонентами shadcn, но в коде проекта их не используем.

#### Уровни и их роли

| Уровень | CSS-переменная | Light | Dark | Роль |
|---------|---------------|-------|------|------|
| 1 | `--rm-gray-1` | `#FAFAFA` | `#121212` | Карточки, поповеры — `card`, `popover` |
| 2 | `--rm-gray-2` | `#F5F5F5` | `#1A1A1A` | Тихая поверхность (badge, инлайн-код, неактивные зоны) |
| 3 | `--rm-gray-3` | `#EBEBEB` | `#242424` | Hover-фон компонентов |
...
|-------------|----------|-----------|----------|----------|
| `background` | `--rm-gray-1` | `#FAFAFA` | `#0A0A0A` | Основной фон страницы |
...
:root {
  /* === Gray Scale (Light) ===
   * 1 — card/popover surface · 2 — muted/secondary · 3 — hover bg (accent)
  ...
  */
  ...
  /* shadcn internals — не использовать в коде проекта напрямую */
  --muted: var(--rm-gray-2);
  --secondary: var(--rm-gray-2);
