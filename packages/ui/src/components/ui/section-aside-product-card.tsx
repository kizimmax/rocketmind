...
function Avatar({ name, image }: { name: string; image: string | null }) {
  ...
  const initials = name
    ...
    .toUpperCase()
  return (
    <span className="flex h-14 w-14 items-center justify-center rounded-full border-2 border-[#0A0A0A] bg-[#333333] text-[length:var(--text-18)] font-medium text-[color:var(--rm-gray-fg-main)]">
      {initials}
    </span>
  )
}
