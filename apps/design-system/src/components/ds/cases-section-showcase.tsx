...
type CaseStat = { value: string; label: string; description: string };
type Case = { title: string; description: string; stats: [CaseStat, CaseStat, CaseStat]; result: string };
...
const CASES: Case[] = [
  {
    ...
    stats: [
      { value: "50",  label: "участников\nпроекта",   description: "Специалисты из четырёх регионов работали в единой методологии — сессии велись на двух языках одновременно" },
      { value: "4",   label: "региона\nмира",          description: "Объединили команды для совместной выработки стратегии и переосмысления продуктового портфеля компании" },
      { value: "20+", label: "бизнес-\nинициатив",     description: "Сгенерировали гипотезы с бизнес-моделями, оценили потенциал и риски для каждого из направлений роста" },
    ],
    result: "Команда освоила клиентоцентричный подход и получила рабочую методологию для оценки новых продуктовых идей.",
  },
  {
    ...
    stats: [
      { value: "30+", label: "глубинных\nинтервью",    description: "Изучили потребности клиентов на всех этапах: от выбора объекта до проживания, выявили ключевые зоны роста" },
      { value: "12",  label: "конкурентов\nизучено",   description: "Проанализировали конкурентный ландшафт и выявили белые пятна рынка для уникального позиционирования" },
      { value: "500+",label: "источников\nинформации", description: "Провели масштабное исследование трендов рынка недвижимости и экосистемных моделей в смежных отраслях" },
    ],
    result: "Создан отдел экосистемных сервисов, внедрена цифровая автоматизация приёмки квартир. Дорожная карта до 2027 года.",
  },
  {
  ...
  },
  {
    ...
    stats: [
      { value: "10",  label: "карт\nклиентского пути", description: "Описали опыт клиента на критических этапах — от первого контакта до получения банковской услуги" },
      { value: "500+",label: "опросов\nклиентов",      description: "Собрали данные для значимых выводов о причинах отказа и падения клиентской лояльности в ключевых сегментах" },
      { value: "15",  label: "ключевых\nбарьеров",     description: "Ранжировали проблемные зоны по влиянию на конверсию и определили приоритеты для устранения барьеров" },
    ],
    result: "Банк получил детальный отчёт с приоритизированными рекомендациями по повышению конверсии и снижению оттока.",
  },
  {
  ...
  return (
    <div className="flex flex-col gap-4 w-full lg:w-[320px]">
      ...
      <div
        ...
        style={{
          maskImage: "linear-gradient(180deg, transparent 0px, #000 64px, #000 calc(100% - 64px), transparent 100%)",
          WebkitMaskImage: "linear-gradient(180deg, transparent 0px, #000 64px, #000 calc(100% - 64px), transparent 100%)",
        }}
          ...
          {[...TESTIMONIALS, ...TESTIMONIALS].map((t, i) => (
            <div key={i}>
              ...
              <div className="text-[14px] leading-[1.4] tracking-[0.01em] text-[#939393]">
                {t.text.split("\n\n").map((para, pi) => (
                  <p key={pi} className={pi > 0 ? "mt-2" : ""}>{nb(para.replace(/\n/g, " "))}</p>
                ))}
              </div>
              {i < TESTIMONIALS.length * 2 - 1 && <div className="h-px bg-[#404040] my-5" />}
            </div>
          ))}
        </div>
      </div>
        ...
        <span key={i} className="flex items-center gap-4">
          <button
            ...
            className={[
              ...
              i === activeCase ? "text-[#F0F0F0]" : "text-[#939393] hover:text-[#F0F0F0]",
            ].join(" ")}
          >
