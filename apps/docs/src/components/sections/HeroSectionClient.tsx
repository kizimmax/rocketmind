...
const LENS_MAX_CAPTURE_SCALE = 1.5;
...
function LogoSequence({ logos }: { logos: PartnerLogo[] }) {
  return (
    <div className="flex shrink-0 items-center gap-[67px] py-[10px] pl-1 pr-11">
      {logos.map((logo) => {
        ...
        return (
          ...
          >
            <Image
              ...
              width={size.width}
              height={size.height}
              className="h-auto w-auto max-h-[39px]"
            />
          </div>
        );
      })}
...
function LogoRail({ logos }: { logos: PartnerLogo[] }) {
  return (
    <div
      data-lens-hide="true"
    ...
    >
      <div
        className="partner-logo-marquee-track"
        style={{ "--hero-marquee-duration": "28s" } as CSSProperties}
      >
...
export function HeroSectionClient({ logos }: HeroSectionClientProps) {
  ...
  const [leavingRotatingLineIndex, setLeavingRotatingLineIndex] = useState<
    number | null
  >(null);
  const [rotatingLineAnimationCycle, setRotatingLineAnimationCycle] =
    useState(0);
  ...
  useEffect(() => {
    ...
    const cleanupStaticLens = () => {
      ...
      const wordmarkImage =
        wordmark.querySelector("img") ?? wordmark.querySelector("svg");
      const wordmarkRect =
        wordmarkImage?.getBoundingClientRect() ?? wordmark.getBoundingClientRect();
      ...
      const offsetX = parentRect.left - heroRect.left;
      const offsetY = parentRect.top - heroRect.top;
      glass.style.left = `${primaryLensLeft - offsetX}px`;
      glass.style.top = `${primaryLensTop - offsetY}px`;
      staticGlass.style.left = `${heroRect.width + 136 - offsetX}px`;
      staticGlass.style.top = `${primaryLensTop + STATIC_LENS_VERTICAL_OFFSET - offsetY}px`;
    ...
    const syncBasePosition = () => {
      ...
      const offsetX = parentRect.left - heroRect.left;
      const offsetY = parentRect.top - heroRect.top;
      glass.style.left = `${base.x - offsetX}px`;
      glass.style.top = `${base.y - offsetY}px`;
      staticGlass.style.left = `${staticBase.x - offsetX}px`;
      staticGlass.style.top = `${staticBase.y - offsetY}px`;
    };
    ...
    const handleMove = (event: PointerEvent) => {
      ...
      target.x = Math.max(-limit, Math.min(limit, relativeX * 0.18));
      target.y = Math.max(-limit, Math.min(limit, relativeY * 0.18));
      staticTarget.x = Math.max(-(limit / 3), Math.min(limit / 3, relativeX * 0.06));
      staticTarget.y = Math.max(-(limit / 3), Math.min(limit / 3, relativeY * 0.06));
      scheduleRender();
      requestCapture();
    };
  ...
  useEffect(() => {
    ...
    const advanceLine = () => {
      setActiveRotatingLineIndex((currentIndex) => {
        const nextIndex =
          (currentIndex + 1) % rocketmindHeroRotatingLines.length;

        setLeavingRotatingLineIndex(currentIndex);
        setRotatingLineAnimationCycle((cycle) => cycle + 1);

        window.clearTimeout(cleanupTimeoutId);
        cleanupTimeoutId = window.setTimeout(() => {
          setLeavingRotatingLineIndex(null);
        }, HERO_ROTATION_TRANSITION_MS + HERO_ROTATION_ENTRY_DELAY_MS);

        return nextIndex;
      });
    };

    intervalId = window.setInterval(advanceLine, HERO_ROTATION_INTERVAL_MS);
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-[1512px] flex-col px-5 pb-10 pt-10 md:px-8 xl:px-14">
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,888px)_212px] lg:items-end lg:justify-between">
          <div className="flex flex-col items-start gap-10">
            <div className="w-full max-w-[888px] font-heading text-[52px] font-bold uppercase leading-[1.08] tracking-[-0.02em]">
              <p className="text-foreground">Помогаем бизнесу расти</p>
              <p className="text-foreground">и масштабироваться</p>
              <div
                className="hero-rotating-line-viewport relative text-muted-foreground"
                style={heroRotatingLineViewportStyle}
                aria-live="off"
              >
                {leavingRotatingLineIndex !== null ? (
                  <p
                    key={`rotating-line-out-${rotatingLineAnimationCycle}-${leavingRotatingLineIndex}`}
                    className="hero-rotating-line-layer hero-rotating-line-animate-out"
                  >
                    {rocketmindHeroRotatingLines[leavingRotatingLineIndex]}
                  </p>
                ) : null}

                <p
                  key={`rotating-line-in-${rotatingLineAnimationCycle}-${activeRotatingLineIndex}`}
                  className={[
                    "hero-rotating-line-layer",
                    leavingRotatingLineIndex !== null
                      ? "hero-rotating-line-animate-in"
                      : "hero-rotating-line-static",
                  ].join(" ")}
                  style={
                    leavingRotatingLineIndex !== null
                      ? {
                          animationDelay: `${HERO_ROTATION_ENTRY_DELAY_MS}ms`,
                        }
                      : undefined
                  }
                >
