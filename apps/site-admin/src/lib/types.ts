...
export interface GlossaryTerm {
  ...
  sections: ArticleSection[];
  /**
   * Альтернативные формы названия (синонимы/падежи/аббревиатуры). Используются
   * для автоподсветки термина в теле статей наряду с `title`. Хранятся в
   * `content.aliases` (миграция БД не требуется).
   */
  aliases: string[];
  /**
   * Авто-сгенерированные падежные формы title — пишутся `glossary-morph`
   * на сохранении, читаются вместе с aliases для авто-подсветки. Пользователь
   * напрямую не редактирует, но видит в превью под полем aliases.
   */
  autoAliases: string[];
  /**
   * Грамматический род последнего слова title. Нужен для генерации падежей
   * через russian-nouns-js. По умолчанию угадывается по окончанию title.
   */
