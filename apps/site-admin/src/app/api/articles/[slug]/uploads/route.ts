...
export async function POST(
  ...
  const kind = rec.kind;
  if (kind !== "file" && kind !== "preview" && kind !== "video" && kind !== "audio") {
    return NextResponse.json({ error: "kind must be 'file' | 'preview' | 'video' | 'audio'" }, { status: 400 });
  }
  ...
  const mapping = kind === "file" ? FILE_MIME_TO_EXT
    : kind === "video" ? VIDEO_MIME_TO_EXT
    : kind === "audio" ? AUDIO_MIME_TO_EXT
    : IMAGE_MIME_TO_EXT;
